...
export function TaskQueue({ version }: { version: number }) {
    ...
    return <section className="panel"><h2>Черга завдань</h2><p className="muted">Оновлюється кожні 5 секунд. Зупинка спрацьовує між запитами до ШІ.</p>
        ...
        {tasks.map(task => {
            const failure = task.message ? describeTaskFailure(task.message) : null;
            return <article className="task-card" key={task.id}><div className="button-row"><strong>{task.novel_id} · {task.operation}</strong><span className="badge">{states[task.state] || task.state}</span></div>
                ...
                {task.current_job_id && <p>ID перекладу: <code>{task.current_job_id}</code></p>}{failure && <div className="task-message" role={task.state === 'failed' ? 'alert' : undefined}><strong>{failure.title}</strong><p>{failure.detail}</p></div>}
