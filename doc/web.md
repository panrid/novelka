...
- Розмір тексту 16–28 px і спільна для всього сайту тема. Темна тема увімкнена
  за замовчуванням; перемикач у шапці сайту і читалці зберігає вибір локально.
...
Власник також бачить баланс OpenRouter у налаштуваннях, якщо сервер має окремий
management key. Черга майстерні показує короткий стан і витрати кожного завдання;
...
|---|---|
| GET /api/novels | NovelCard[]: id, title, author, description, chapterCount, readyChapters, aliases |
| GET /api/novels/{idOrAlias} | NovelDetail: title, author, description, chapterCount та ChapterSummary[] |
| GET /api/novels/{idOrAlias}/chapters/{number} | ReaderChapter: novelId, number, revision, title, blocks, jobId, personalReplacements |
...
Для росту каталогу знадобляться серверна пагінація та пошук; акаунти, правки,
аудит і черга вже мають сторінки по 50 записів. Наразі воркер виконує одне
